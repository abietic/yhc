# Implementation Plan Index

**Status:** active-plan
**Last verified:** 2026-08-13

> **Ownership:** discoverability and lifecycle routing for task-level
> implementation plans

| Plan | Owning accepted contract | State |
|---|---|---|
| [`2026-08-13-untracked-sbom.md`](2026-08-13-untracked-sbom.md) | [Untracked SBOM design](../specs/2026-08-13-untracked-sbom-design.md) | Historical; implemented and locally verified 2026-08-13 |
| [`2026-08-09-yhc-public-release-program.md`](2026-08-09-yhc-public-release-program.md) | [YHC Public Release And Identity Migration design](../specs/2026-08-09-yhc-public-release-design.md) | Historical; completed 2026-08-11 |
| [`2026-08-09-yhc-publication-readiness.md`](2026-08-09-yhc-publication-readiness.md) | [YHC publication boundary, security gates, and governance](../specs/2026-08-09-yhc-public-release-design.md#public-content-boundary) | Executed and retained as closeout evidence |
| [`2026-08-09-yhc-core-identity.md`](2026-08-09-yhc-core-identity.md) | [YHC identity and environment compatibility contract](../specs/2026-08-09-yhc-public-release-design.md#identity-contract) | Historical; completed 2026-08-11 |
| [`2026-08-09-yhc-state-foundation.md`](2026-08-09-yhc-state-foundation.md) | [YHC state-owner compatibility rules](../specs/2026-08-09-yhc-public-release-design.md#state-owners-migrate-their-own-artifacts) | Historical; completed 2026-08-11 |
| [`2026-08-09-yhc-state-continuity.md`](2026-08-09-yhc-state-continuity.md) | [YHC session, cron, and worktree continuity rules](../specs/2026-08-09-yhc-public-release-design.md#state-owners-migrate-their-own-artifacts) | Historical; completed 2026-08-11 |
| [`2026-08-09-yhc-protocol-compatibility.md`](2026-08-09-yhc-protocol-compatibility.md) | [YHC ACP and MCP compatibility contract](../specs/2026-08-09-yhc-public-release-design.md#acp-selects-one-goal-namespace-per-connection) | Historical; completed 2026-08-11 |
| [`2026-08-09-yhc-clean-root-cutover.md`](2026-08-09-yhc-clean-root-cutover.md) | [YHC repository topology, promotion, and public acceptance](../specs/2026-08-09-yhc-public-release-design.md#repository-topology-and-cutover) | Historical; completed 2026-08-11 |
| [`2026-08-09-iteration-quality-s5a-context-workspace.md`](2026-08-09-iteration-quality-s5a-context-workspace.md) | [S5 completion design](../specs/2026-08-09-iteration-quality-s5-completion-design.md) | Active; F0 executed, followed by A1-A3 |
| [`2026-08-09-iteration-quality-s5b-regression-oracles.md`](2026-08-09-iteration-quality-s5b-regression-oracles.md) | [S5 completion design](../specs/2026-08-09-iteration-quality-s5-completion-design.md) | Ready after executed F0; B1 is the first regression slice |
| [`2026-08-09-iteration-quality-s5c-measurement-readiness.md`](2026-08-09-iteration-quality-s5c-measurement-readiness.md) | [S5 completion design](../specs/2026-08-09-iteration-quality-s5-completion-design.md) | Ready after executed F0; C1 is the first measurement slice |
| [`2026-08-09-todo-workboard-transcript-mode-hotfix.md`](2026-08-09-todo-workboard-transcript-mode-hotfix.md) | [Todo and WorkBoard transcript mode hotfix design](../specs/2026-08-08-todo-workboard-transcript-mode-hotfix-design.md) | Executed and retained as closeout evidence |
| [`2026-08-08-iteration-quality-s1-policy-planner.md`](2026-08-08-iteration-quality-s1-policy-planner.md) | [Iteration Quality Kernel design](../specs/2026-08-08-iteration-quality-kernel-design.md) | Executed; strict policy, read-only planner, and initial evidence schema |
| [`2026-08-08-iteration-quality-s2-verification-e2e.md`](2026-08-08-iteration-quality-s2-verification-e2e.md) | [Iteration Quality Kernel design](../specs/2026-08-08-iteration-quality-kernel-design.md) | Executed; diff-bound focused and merge evidence, advisory coverage, and deterministic real-binary E2E |
| [`2026-08-08-iteration-quality-s3-context-workflow.md`](2026-08-08-iteration-quality-s3-context-workflow.md) | [Iteration Quality Kernel design](../specs/2026-08-08-iteration-quality-kernel-design.md) | Executed; shared workflow, optional hooks, pre-push evidence, and retired compatibility alias |
| [`2026-08-08-iteration-quality-s4-defect-boundaries.md`](2026-08-08-iteration-quality-s4-defect-boundaries.md) | [Iteration Quality Kernel design](../specs/2026-08-08-iteration-quality-kernel-design.md) | Executed for defect discovery and new-edge boundary governance; product-module acceptance remains intake-gated |
| [`2026-08-07-p47-7-lazy-execution-detail.md`](2026-08-07-p47-7-lazy-execution-detail.md) | [`P47.7`](../../migration/plans/p47-task-explorer-remediation.md#p477-lazy-execution-detail) | Executed and retained as closeout evidence |
| [`2026-08-07-p47-6-snapshot-detail.md`](2026-08-07-p47-6-snapshot-detail.md) | [`P47.6`](../../migration/plans/p47-task-explorer-remediation.md#p476-snapshot-detail-structure) | Executed and retained as closeout evidence |
| [`2026-08-07-p47-5-filter-focus.md`](2026-08-07-p47-5-filter-focus.md) | [`P47.5`](../../migration/plans/p47-task-explorer-remediation.md#p475-filter-and-focus-navigation) | Executed and retained as closeout evidence |
| [`2026-08-07-p47-4-mixed-rows.md`](2026-08-07-p47-4-mixed-rows.md) | [`P47.4`](../../migration/plans/p47-task-explorer-remediation.md#p474-mixed-rows-and-stable-selection) | Executed and retained as closeout evidence |
| [`2026-08-07-p47-3-exact-thread-navigation.md`](2026-08-07-p47-3-exact-thread-navigation.md) | [`P47.3`](../../migration/plans/p47-task-explorer-remediation.md#p473-exact-thread-navigation-target) | Executed and retained as closeout evidence |
| [`2026-08-07-p47-2-workitem-scoped-settlement.md`](2026-08-07-p47-2-workitem-scoped-settlement.md) | [`P47.2`](../../migration/plans/p47-task-explorer-remediation.md#p472-workitem-scoped-settlement) | Executed and retained as closeout evidence |
| [`2026-08-07-p49-goal-default-unbudgeted.md`](2026-08-07-p49-goal-default-unbudgeted.md) | [`P49`](../../migration/plans/p49-goal-default-unbudgeted.md#atomic-implementation-boundary) | Executed and retained as closeout evidence |
| [`2026-08-07-p47-1-task-explorer-action-intent.md`](2026-08-07-p47-1-task-explorer-action-intent.md) | [`P47.1`](../../migration/plans/p47-task-explorer-remediation.md#p471-exact-pending-action-intent) | Executed and retained as closeout evidence |
| [`2026-08-07-p48-1-acp-session-root-delete.md`](2026-08-07-p48-1-acp-session-root-delete.md) | [`P48.1`](../../migration/plans/p48-acp-boundary-remediation.md#p481-observed-session-root-delete) | Executed and retained as closeout evidence |
| [`2026-08-07-p48-2-acp-plan-tool-identity.md`](2026-08-07-p48-2-acp-plan-tool-identity.md) | [`P48.2`](../../migration/plans/p48-acp-boundary-remediation.md#p482-plan-tool-call-identity) | Executed and retained as closeout evidence |
| [`2026-08-07-p48-3-acp-string-raw-output.md`](2026-08-07-p48-3-acp-string-raw-output.md) | [`P48.3`](../../migration/plans/p48-acp-boundary-remediation.md#p483-string-tool-raw-output) | Executed and retained as closeout evidence |
| [`2026-08-07-p48-4-mcp-environment-identity.md`](2026-08-07-p48-4-mcp-environment-identity.md) | [`P48.4`](../../migration/plans/p48-acp-boundary-remediation.md#p484-os-aware-mcp-environment-identity) | Executed and retained as closeout evidence |
| [`2026-08-07-p48-5-remove-private-session-migration.md`](2026-08-07-p48-5-remove-private-session-migration.md) | [`P48.5`](../../migration/plans/p48-acp-boundary-remediation.md#p485-remove-private-session-migration) | Executed and retained as closeout evidence |
| [`2026-08-07-p50-1-project-graph-revision-fence.md`](2026-08-07-p50-1-project-graph-revision-fence.md) | [Permission Runtime Remediation design](../specs/2026-08-07-permission-remediation-design.md#repair-1-fence-projectgraph-settlement-at-the-rebuilt-revision) | Executed and retained as closeout evidence |
| [`2026-08-07-p50-2-reviewer-latency-denominator.md`](2026-08-07-p50-2-reviewer-latency-denominator.md) | [Permission Runtime Remediation design](../specs/2026-08-07-permission-remediation-design.md#repair-2-use-reviewer-attempts-as-the-latency-denominator) | Executed and retained as closeout evidence |
| [`2026-08-07-p50-3-review-audit-dispatcher.md`](2026-08-07-p50-3-review-audit-dispatcher.md) | [Permission Runtime Remediation design](../specs/2026-08-07-permission-remediation-design.md#repair-3-queue-reviewer-audit-without-blocking-permission) | Historical; completed 2026-08-08 |
| [`2026-08-07-p51-1-darwin-guest-seatbelt.md`](2026-08-07-p51-1-darwin-guest-seatbelt.md) | [Darwin Sandbox and Auto Permission design](../specs/2026-08-07-darwin-sandbox-auto-permission-design.md#delivery-decomposition) | Executed; completed and removed from the migration queue |
| [`2026-08-07-p51-2-auto-containment-admission.md`](2026-08-07-p51-2-auto-containment-admission.md) | [Darwin Sandbox and Auto Permission design](../specs/2026-08-07-darwin-sandbox-auto-permission-design.md#auto-permission-admission) | Historical; superseded by the Desktop-aware public forward-port plan |
| [`2026-08-07-agent-runtime-instruction-freshness.md`](2026-08-07-agent-runtime-instruction-freshness.md) | [Agent Runtime Instruction Freshness design](../specs/2026-08-07-agent-runtime-instruction-freshness-design.md) | Executed and retained as closeout evidence |
| [`2026-08-07-closed-gap-traceability.md`](2026-08-07-closed-gap-traceability.md) | [Closed Gap Traceability design](../specs/2026-08-07-closed-gap-traceability-design.md) | Executed and retained as closeout evidence |
| [`2026-08-06-p46-2-observable-failover.md`](2026-08-06-p46-2-observable-failover.md) | [`P46.2`](../../migration/plans/p46-model-failover-repair.md#p462-observable-attempt-discard-and-switch) | Executed and retained as closeout evidence |
| [`2026-08-06-p46-1-complete-prompt-footprint.md`](2026-08-06-p46-1-complete-prompt-footprint.md) | [`P46.1`](../../migration/plans/p46-model-failover-repair.md#p461-complete-prompt-footprint-admission) | Executed and retained as closeout evidence |
| [`2026-08-13-desktop-session-creation-activation.md`](2026-08-13-desktop-session-creation-activation.md) | [Desktop Session Creation Activation design](../specs/2026-08-13-desktop-session-creation-activation-design.md) | Active; implementation and focused verification complete, committed-tree and physical acceptance remain |
| [`2026-08-13-yhc-desktop-workbench-forward-port.md`](2026-08-13-yhc-desktop-workbench-forward-port.md) | [YHC Desktop Workbench Forward-Port design](../specs/2026-08-13-yhc-desktop-workbench-forward-port-design.md) | Active; implementation complete, final committed-tree and physical acceptance remain |
| [`2026-08-13-proof-bound-auto-bash-forward-port.md`](2026-08-13-proof-bound-auto-bash-forward-port.md) | [Proof-Bound Auto Bash Public Forward-Port design](../specs/2026-08-13-proof-bound-auto-bash-forward-port-design.md) | Historical; implemented and locally verified 2026-08-13 |

Boundary enforcement and deep defect discovery are implemented. Overall S4
product-module acceptance remains open until an accepted `Ready` product
slice touches a broad owner and demonstrates behavior-preserving deepening in
its own test-first plan. Tooling cleanup, file-size reduction, or a deferred
gap does not satisfy that gate. The migration queue remains the ordering SSOT;
this terminal governance state neither repopulates it nor implies that P44 is
ready.
