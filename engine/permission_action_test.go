...
func TestP512PermissionRewriteCannotEscalateOrdinaryBashToCritical(
	t *testing.T,
) {
	for _, decision := range []PermissionInteractionDecision{
		...
		t.Run(string(decision), func(t *testing.T) {
			...
			outcome := executeToolCall(
				...
				&schema.ToolCall{
					ID: "rewrite-critical-" + string(decision),
					Function: schema.FunctionCall{
						Name:      "Bash",
						Arguments: `{"command":"printf ordinary"}`,
					},
				},
				nil,
			)
			if outcome == nil || outcome.Result == nil ||
				...
				!strings.Contains(
					...
					"sandbox_critical_path_confirmation_required",
				) {
				t.Fatalf("outcome = %#v", outcome)
			}
			if prompts.Load() != 1 || executions.Load() != 0 {
				t.Fatalf(
					"prompt/execution = %d/%d, want 1/0",
					prompts.Load(),
					executions.Load(),
				)
			}
			if approvals := engine.approvalTracker.List(); len(approvals) != 0 {
				t.Fatalf("critical rewrite committed session grant: %#v", approvals)
			}
			if rules := engine.permissionRulesSnapshot().Snapshot(); len(rules) != 0 {
				t.Fatalf("critical rewrite persisted allow rule: %#v", rules)
			}
		})
	}
}
