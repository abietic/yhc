// Modified by YHC from acp-go-sdk v0.13.5: inbound requests wait only for
// notifications accepted before the request, with a deterministic test seam.
package acp
...
type Connection struct {
	...
	// inboundRequestBarrierHook observes an inbound request reaching its
	// notification barrier. It is package-private so SDK tests can coordinate
	// deterministic ordering proofs without changing the public API.
	inboundRequestBarrierHook func()
}

func NewConnection(handler MethodHandler, peerInput io.Writer, peerOutput io.Reader) *Connection {
	return newConnection(handler, peerInput, peerOutput, nil)
}

func newConnection(handler MethodHandler, peerInput io.Writer, peerOutput io.Reader, inboundRequestBarrierHook func()) *Connection {
	...
	c := &Connection{
		...
		inboundRequestBarrierHook: inboundRequestBarrierHook,
	}
...
func (c *Connection) receive() {
	...
	for scanner.Scan() {
		...
		case msg.Method != "":
			if msg.ID != nil {
				// Requests observe the notification queue as it stood when they arrived.
				// Later notifications must not delay this request, while notifications already
				// accepted onto the queue must finish before its handler starts.
				c.notifyMu.Lock()
				...
				m := msg
				go func(m *anyMessage, idKey string, reqCtx context.Context, cancel context.CancelCauseFunc, watermark uint64) {
					defer func() {
						c.mu.Lock()
					...
					if err := c.waitInboundNotificationsUpTo(reqCtx, watermark); err != nil {
						c.sendInboundRequestError(m, toReqErr(err))
						return
					}
					c.handleInbound(reqCtx, m)
				}(&m, idKey, reqCtx, cancel, watermark)
				continue
			}
...
func (c *Connection) waitInboundNotificationsUpTo(ctx context.Context, target uint64) error {
	return c.waitNotificationsUpTo(ctx, target, c.inboundRequestBarrierHook)
}
...
func (c *Connection) sendInboundRequestError(req *anyMessage, err *RequestError) {
	if req.ID == nil {
		return
	}
	_ = c.sendMessage(anyMessage{
		JSONRPC: "2.0",
		ID:      req.ID,
		Error:   err,
	})
...
func SendRequest[T any](c *Connection, ctx context.Context, method string, params any) (T, error) {
	...
	if err := c.waitNotificationsUpTo(ctx, resp.notificationWatermark, nil); err != nil {
		return result, err
	}
...
func (c *Connection) waitNotificationsUpTo(ctx context.Context, target uint64, reachedBarrier func()) error {
	if target == 0 {
		if reachedBarrier != nil {
			reachedBarrier()
		}
		return nil
	}
