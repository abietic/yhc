...
func TestPolicyClassifiesPublicationManifestAsGovernance(t *testing.T) {
	root := openPolicyRoot(t, "../..")
	policy, err := loadPolicy(root, "quality/iteration.yaml")
	if err != nil {
		t.Fatal(err)
	}
	candidates, err := ownerCandidatesForPath(policy, "PUBLICATION_MANIFEST.json")
	if err != nil {
		t.Fatal(err)
	}
	winner, err := chooseOwner("PUBLICATION_MANIFEST.json", candidates)
	if err != nil {
		t.Fatal(err)
	}
	if winner.owner != "governance" || winner.kind != PathClass {
		t.Fatalf("publication manifest owner = %s/%s, want governance/%s", winner.owner, winner.kind, PathClass)
	}
}
